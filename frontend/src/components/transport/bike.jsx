...
import { getAllBikeOrder,deleteBikeOrder } from '../../Api';
...
const OrderTable = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await getAllBikeOrder();
            setOrders(response);
            setLoading(false);
        } catch (err) {
                    ...
                    {filteredOrders.length === 0 ? (
                        ...
                        filteredOrders.map((order, index) => (
                            <div className="table-row" key={index}>
                                ...
                                <div data-label="Bick Type">{order.bikeType}</div>
