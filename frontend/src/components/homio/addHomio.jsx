...
const AmbulanceForm = () => {
    ...
    const validateForm = () => {
        ...
        if (!formData.name.trim()) {
            newErrors.name = 'Homio name is required';
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await createHomio(formData);
            setFormData({ name: '', number: '', location: '', time: '' });
            alert('Homio added successfully!');
        } catch (error) {
            // Display error message based on server response if available
            alert(`Error: ${error.response?.data?.message || 'Failed to add Homio'}`);
        } finally {
    ...
    return (
        <div className="max-w-[800px] !m-auto !p-6 !mt-8 bg-white rounded-lg shadow-md">
            <h2 className="text-2xl font-bold mb-6 text-gray-800">Add New Homio</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                >
                    {isSubmitting ? 'Submitting...' : 'Add Homio'}
                </button>
            </form>
        </div>
    );
