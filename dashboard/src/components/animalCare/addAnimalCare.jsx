...
const AmbulanceForm = () => {
    ...
    const validateForm = () => {
        ...
        if (!formData.name.trim()) {
            newErrors.name = 'Animal Care name is required';
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await createAnimalCare(formData);
            setFormData({ name: '', number: '', location: '', time: '' });
            alert('Animal Care added successfully!');
        } catch (error) {
            // Display error message based on server response if available
            alert(`Error: ${error.response?.data?.message || 'Failed to add animal care'}`);
        } finally {
    ...
    return (
        <div className="max-w-[800px] !m-auto !p-6 !mt-8 bg-white rounded-lg shadow-md">
            <h2 className="text-2xl font-bold mb-6 text-gray-800">Add New Animal Care</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                >
                    {isSubmitting ? 'Submitting...' : 'Add Animal Care'}
                </button>
            </form>
        </div>
    );
