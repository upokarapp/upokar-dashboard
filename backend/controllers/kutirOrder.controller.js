...
export const getAllKutirOrders = async (req, res) => {
    try {
        const orders = await KutirOrder.aggregate([
            // Lookup to join with the products collection based on order.pid
            {
                $lookup: {
                    from: 'kutirshilpos', // ensure this matches your actual collection name
...
export const deleteKutirOrder = async (req, res) => {
    const id = req.params.id;

    try {
        const product = await KutirOrder.findByIdAndDelete(id);
        if (!product) {
