...
const login = async (req, res) => {
  try {
    ...
    if (!name || !password) {
      return res
        .status(400)
        .json({ message: "Please enter name and password" });
    }
...
const createAdmin = async (req, res) => {
  try {
    ...
    if (!name || !password) {
      return res
        .status(400)
        .json({ message: "Please enter name and password for the admin" });
    }
    ...
    if (existingAdmin) {
      return res
        .status(409)
        .json({ message: "Admin with this name already exists" }); // 409 Conflict
    }
